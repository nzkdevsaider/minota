...
import UpdateCourseNameDialog from "../Dialogs/UpdateCourseNameDialog";
import UpdateItemNameDialog from "../Dialogs/UpdateItemNameDialog";
		...
		<section className="space-y-8">
			...
			<div className="flex flex-col justify-center items-center gap-5">
				<div className="flex flex-row justify-center items-center gap-3">
					...
					<button
						type="button"
						onClick={() => window.updateCourseName.showModal()}
				...
				items.map((item) => (
					...
					>
						<div className="flex flex-nowrap items-center justify-between">
							...
							<div key={item.id}>
								<UpdateItemNameDialog itemId={item.id} />
								<label
									htmlFor={`updateItemName_${item.id}`}
